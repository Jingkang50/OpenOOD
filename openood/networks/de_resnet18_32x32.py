...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, upsample = None):
        ...
        self.stride = stride
        if self.stride == 2:
            self.conv1 = nn.ConvTranspose2d(in_planes, planes, kernel_size=2, stride=stride,bias=False)    
        else:
...
class De_ResNet18_32x32(nn.Module):
    def __init__(self, block = BasicBlock, num_blocks = None, num_classes=10):
        super(De_ResNet18_32x32, self).__init__()
...
class BN_layer(nn.Module):
    def __init__(self,
        ...
        self.conv4 = nn.Conv2d(1024 * block.expansion, 512 * block.expansion, kernel_size = 1,stride = 1, bias = False)
    ...
    def _make_layer(self, block, planes: int, blocks: int,
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes*3, planes * block.expansion, kernel_size = 1, stride = stride, bias = False),
                norm_layer(planes * block.expansion),
            )
