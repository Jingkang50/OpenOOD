...
from timm.models.features import FeatureDictNet, FeatureHookNet, FeatureListNet
from timm.models.fx_features import FeatureGraphNet
from timm.models.hub import (download_cached_file, has_hf_hub,
...
def load_custom_pretrained(model,
    ...
    if not pretrained_url:
        ...
        return
    cached_file = download_cached_file(default_cfg['url'],
        ...
        for r in range(2):
            if hasattr(block.mlp, 'experts'):
                ...
                for ind in range(num_experts):
                    getattr(block.mlp.experts, f'w{r + 1}')[ind].copy_(
                        torch.from_numpy(
                            w[f'{block_prefix}MlpBlock_3/Dense_{r}/kernel']))
            else:
    ...
    for i, block in enumerate(model.blocks.children()):
        if hasattr(block.mlp, 'experts'):
            ...
            if isinstance(block.mlp, moe_layer.MOELayer):
                ...
                ## fc1
                hidden_dim, input_dim = state_dict[
                    f'blocks.{i}.mlp.fc1.weight'].shape
                state_dict[
                    f'blocks.{i}.mlp.experts.batched_fc1_w'] = state_dict[
                        f'blocks.{i}.mlp.fc1.weight'].expand(
                            num_experts, hidden_dim, input_dim)
                state_dict[
                    f'blocks.{i}.mlp.experts.batched_fc2_w'] = state_dict[
                        f'blocks.{i}.mlp.fc2.weight'].expand(
                            num_experts, input_dim,
                            hidden_dim).permute(0, 2, 1)
                for r in range(1, 3):
                    hidden_dim, input_dim = state_dict[
                        f'blocks.{i}.mlp.fc{r}.weight'].shape
                    vit_mlp_bias = state_dict[f'blocks.{i}.mlp.fc{r}.bias']
                    stacked_vit_mlp_bias = vit_mlp_bias.expand(
                        num_experts, 1, hidden_dim)
                    state_dict[
                        f'blocks.{i}.mlp.experts.batched_fc{r}_bias'] = stacked_vit_mlp_bias
...
def named_modules(module: nn.Module,
                  name='',
                  depth_first=True,
                  include_root=False):
    if not depth_first and include_root:
    ...
    for child_name, child_module in module.named_children():
        child_name = '.'.join((name, child_name)) if name else child_name
        yield from named_modules(module=child_module,
                                 name=child_name,
                                 depth_first=depth_first,
                                 include_root=True)
    if depth_first and include_root:
