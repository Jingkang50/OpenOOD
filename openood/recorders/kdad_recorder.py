...
class KdadRecorder:
    ...
    def report(self, train_metrics, eval_metrics):
        print('epoch [{}],time:{:5d}s,loss:{:.4f},roc_auc:{:.2f}'.format(
            train_metrics['epoch_idx'], int(time.time() - self.begin_time),
            train_metrics['epoch_loss'], eval_metrics['roc_auc']))

    def save_model(self, net, eval_metrics):
        if self.config.recorder.save_all_models:
            torch.save(
                net['model'].state_dict(),
                os.path.join(
                    self.output_dir,
                    'Clone_epoch{}.ckpt'.format(eval_metrics['epoch_idx'])))

        # enter only if better accuracy occurs
        if eval_metrics['roc_auc'] >= self.best_roc_auc:
            ...
            self.best_epoch_idx = eval_metrics['epoch_idx']
            self.best_roc_auc = eval_metrics['roc_auc']
        ...
        if eval_metrics['epoch_idx'] == self.config['last_checkpoint']:
            torch.save(
                net['model'].state_dict(),
                '{}/Cloner_{}_epoch_{}.pth'.format(self.config['output_dir'],
                                                   self.config.normal_class,
                                                   eval_metrics['epoch_idx']))
