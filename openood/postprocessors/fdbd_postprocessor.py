...
class fDBDPostprocessor(BasePostprocessor):
    ...
    def postprocess(self, net: nn.Module, data: Any):
        ...
        if self.distance_as_normalizer:
          score = torch.sum(logits_sub/self.denominator_matrix[nn_idx], axis=1)/torch.norm(feature - self.train_mean, dim = 1)
        else:
          score = torch.sum(logits_sub/self.denominator_matrix[nn_idx], axis=1)/torch.norm(feature, dim = 1)
        return nn_idx, score
